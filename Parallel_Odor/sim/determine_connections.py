...
granules = Parallel_Odor.params.granules
from Parallel_Odor.common import *
from Parallel_Odor.sim.all2all import all2all
import Parallel_Odor.util as util
from Parallel_Odor.gidfunc import *
import Parallel_Odor.mkmitral as mkmitral
...
import Parallel_Odor.lateral_connections as latconn



gc2nconn = {}
for ggid in granules.ggid2pos.keys():
  if (ggid - Parallel_Odor.params.gid_granule_begin) % nhost == rank:
    gc2nconn[ggid] = 0
...
def glom2ranks(glomid):
  return set([ mgid%nhost for mgid in range(glomid*Parallel_Odor.params.Nmitral_per_glom, (glomid+1)*Parallel_Odor.params.Nmitral_per_glom) ] + \
         [ mtgid%nhost for mtgid in range(glomid*Parallel_Odor.params.Nmtufted_per_glom+Parallel_Odor.params.gid_mtufted_begin, (glomid+1)*Parallel_Odor.params.Nmtufted_per_glom+Parallel_Odor.params.gid_mtufted_begin) ])

def ggid2rank(ggid):
  return (ggid - Parallel_Odor.params.gid_granule_begin) % nhost
...
def mk_mconnection_info(model):
  ...
  for gid in model.mitrals.keys(): #+model.mtufted.keys():
    r[gid] = Parallel_Odor.params.ranstream(gid, Parallel_Odor.params.stream_latdendconnect) # init rng
