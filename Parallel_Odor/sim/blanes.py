...
import Parallel_Odor.misc as misc
import Parallel_Odor.geodist as geodist
...
def mk_gl2b_connections():
  ...
  for x in params.glom2blanes:
    ...
    if blanes_gid not in getmodel().blanes.keys():
      continue
    for mtgid in range(glomid*Parallel_Odor.params.Nmtufted_per_glom+Parallel_Odor.params.gid_mtufted_begin, (glomid+1)*Parallel_Odor.params.Nmtufted_per_glom+Parallel_Odor.params.gid_mtufted_begin):
...
class mt2blanes:
    def __init__(self, mgid, blanesgid, w=None):
        ...
        rng = Parallel_Odor.params.ranstream(blanesgid, Parallel_Odor.params.stream_blanes+gidfunc.mgid2glom(mgid))
...
class blanes2granule:
    def __init__(self, blanesgid, ggid, w=1):
        ...
        rng = Parallel_Odor.params.ranstream(blanesgid, Parallel_Odor.params.stream_blanes+Parallel_Odor.params.Nmtufted+ggid)
