...
'''
import Parallel_Odor.blanes as blanes
from Parallel_Odor.common import *
from Parallel_Odor.util import elapsed
from Parallel_Odor.split import wholemitral, mpiece_exists
t_begin = h.startsw()
import Parallel_Odor.determine_connections as dc
h.load_file("granule.hoc")
from Parallel_Odor.lateral_connections import gc_is_superficial as gcissup

import Parallel_Odor.mgrs as mgrs
elapsed('net_mitral_centric after import mgrs')
import Parallel_Odor.sim.all2all as a2a
import Parallel_Odor.params as params
granules = Parallel_Odor.granules
...
def mkgranule(gid):
  ...
  def randomize_diam(r):
    ...
    rn = r.normal(Parallel_Odor.params.granule_rn_mean, Parallel_Odor.params.granule_rn_std**2)
    while rn < (Parallel_Odor.params.granule_rn_mean - 3*Parallel_Odor.params.granule_rn_std) or \
          rn >  (Parallel_Odor.params.granule_rn_mean + 3*Parallel_Odor.params.granule_rn_std):
      rn = r.repick()
  ...
  g = h.Granule()
  r = Parallel_Odor.params.ranstream(gid, Parallel_Odor.params.stream_granule_diam)
  randomize_diam(r)
  if gcissup(gid):
    g.sup_deep_flag(1)
  else:
...
def build_net_round_robin(model, mitral_gids=set(range(Parallel_Odor.params.Nmitral)), connection_file=''):
  model.clear()
...
if __name__ == '__main__':
  from Parallel_Odor.util import serialize
  model = getmodel()
  for r in serialize():
